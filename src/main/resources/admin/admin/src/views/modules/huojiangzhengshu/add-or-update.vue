<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="24">  
        <el-form-item class="upload" v-if="type!='info' && !ro.zhengshufengmian" label="证书封面" prop="zhengshufengmian">
          <file-upload
          tip="点击上传证书封面"
          action="file/upload"
          :limit="3"
          :multiple="true"
          :fileUrls="ruleForm.zhengshufengmian?ruleForm.zhengshufengmian:''"
          @change="zhengshufengmianUploadChange"
          ></file-upload>
        </el-form-item>
        <div v-else>
          <el-form-item v-if="ruleForm.zhengshufengmian" label="证书封面" prop="zhengshufengmian">
            <img style="margin-right:20px;" v-bind:key="index" v-for="(item,index) in ruleForm.zhengshufengmian.split(',')" :src="item" width="100" height="100">
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
        <el-form-item class="date" v-if="type!='info'" label="获奖日期" prop="huojiangriqi">
            <el-date-picker
                format="yyyy 年 MM 月 dd 日"
                value-format="yyyy-MM-dd"
                v-model="ruleForm.huojiangriqi" 
                type="date"
                placeholder="获奖日期">
            </el-date-picker> 
        </el-form-item>
        <div v-else>
          <el-form-item class="input" v-if="ruleForm.huojiangriqi" label="获奖日期" prop="huojiangriqi">
              <el-input v-model="ruleForm.huojiangriqi" 
                placeholder="获奖日期" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
      ...
      <el-col :span="12">
        ...
        <div v-else>
          ...
          <el-row>
            <el-col :span="24">
              <el-form-item v-if="type!='info'"  label="获奖内容" prop="huojiangneirong">
                <editor 
              ...
              <div v-else>
                <el-form-item v-if="ruleForm.huojiangneirong" label="获奖内容" prop="huojiangneirong">
                    <span v-html="ruleForm.huojiangneirong"></span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
...
export default {
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.jiangxiangmingcheng = true;
          ...
          if(o=='zhengshufengmian'){
            this.ruleForm.zhengshufengmian = obj[o];
	    this.ro.zhengshufengmian = true;
            continue;
          }
          if(o=='huojiangriqi'){
            this.ruleForm.huojiangriqi = obj[o];
	    this.ro.huojiangriqi = true;
            continue;
          }
	...
	this.ruleForm.huojiangneirong = this.ruleForm.huojiangneirong.replace(reg,'../../../springbootx877q/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `huojiangzhengshu/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.huojiangzhengshuCrossAddOrUpdateFlag = false;
                  this.parent.search();
    ...
    zhengshufengmianUploadChange(fileUrls) {
	this.ruleForm.zhengshufengmian = fileUrls;
			this.addEditUploadStyleChange()
    },
	addEditStyleChange() {
	  this.$nextTick(()=>{
