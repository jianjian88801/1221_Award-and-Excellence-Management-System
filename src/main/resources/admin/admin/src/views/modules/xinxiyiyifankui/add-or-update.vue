    ...
    >
      ...
      <el-col :span="12">
        ...
        <div v-else>
          ...
          <el-row>
            <el-col :span="24">
              <el-form-item v-if="type!='info'"  label="异议内容" prop="yiyineirong">
                <editor 
                    style="min-width: 200px; max-width: 600px;"
                    v-model="ruleForm.yiyineirong" 
                    class="editor" 
                    action="file/upload">
                </editor>
              </el-form-item>
              <div v-else>
                <el-form-item v-if="ruleForm.yiyineirong" label="异议内容" prop="yiyineirong">
                    <span v-html="ruleForm.yiyineirong"></span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
...
export default {
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.fankuishijian = true;
          ...
          if(o=='yiyineirong'){
            this.ruleForm.yiyineirong = obj[o];
	    this.ro.yiyineirong = true;
            continue;
          }
          if(o=='huifu'){
	...
	this.ruleForm.yiyineirong = this.ruleForm.yiyineirong.replace(reg,'../../../springbootx877q/upload');
	this.ruleForm.huifu = this.ruleForm.huifu.replace(reg,'../../../springbootx877q/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `xinxiyiyifankui/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.xinxiyiyifankuiCrossAddOrUpdateFlag = false;
                  this.parent.search();
